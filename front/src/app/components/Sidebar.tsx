        ...
        <aside className="w-64 flex-shrink-0 bg-cyan-950 text-gray-100 flex flex-col">
            <div className="flex-grow overflow-y-auto custom-scrollbar-subtle p-6">
                ...
                <nav className="space-y-3">
                    <button onClick={() => router.push("/Principal")} className="w-full flex items-center p-3 hover:bg-cyan-800 rounded-lg">
                        <FaUsers className="mr-4 text-green-400" /> Ver Pacientes
                    </button>

                    <button onClick={() => setShowAddModal(true)} className="w-full flex items-center p-3 hover:bg-cyan-800 rounded-lg">
                        <FaPlusSquare className="mr-4 text-green-400" /> Nuevo Paciente
                    </button>

                    <button onClick={() => router.push("/Principal?view=cirugias")} className="w-full flex items-center p-3 hover:bg-cyan-800 rounded-lg">
                        <FaCalendarAlt className="mr-4 text-green-400" /> Ver cirugías
                    </button>

                    <button onClick={() => setShowCirugiaModal(true)} className="w-full flex items-center p-3 hover:bg-cyan-800 rounded-lg">
                        <FaPlusSquare className="mr-4 text-green-400" /> Agregar cirugía
                    </button>

                    <button className="w-full flex items-center p-3 hover:bg-cyan-800 rounded-lg">
                        <FaMoneyBillAlt className="mr-4 text-green-400" /> Informes de cobros
                    </button>

                    <button className="w-full flex items-center p-3 hover:bg-cyan-800 rounded-lg">
                        <FaChartBar className="mr-4 text-green-400" /> Información
                    </button>
                </nav>
            </div>

            <div className="p-6 border-t border-gray-700">
                <button onClick={handleLogout} className="w-full flex items-center p-3 bg-red-700 hover:bg-red-800 rounded-lg">
