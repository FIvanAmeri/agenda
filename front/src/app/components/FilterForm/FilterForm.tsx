...
    const names = new Set(dataToProcess.map((patient) => patient.paciente));
    return Array.from(names).sort();
  }, [dataToProcess]);

  const practices = useMemo(() => {
    const practicesSet = new Set(dataToProcess.map((patient) => patient.practicas));
    return Array.from(practicesSet).sort();
  }, [dataToProcess]);

  const obrasSociales = useMemo(() => {
    const obrasSet = new Set(dataToProcess.map((patient) => patient.obraSocial));
    return Array.from(obrasSet).sort();
  }, [dataToProcess]);

  const instituciones = useMemo(() => {
    const institucionesSet = new Set(dataToProcess.map((patient) => patient.institucion));
