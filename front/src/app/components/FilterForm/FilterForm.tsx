...
const FilterForm: React.FC<FilterFormProps> = ({
    ...
    const fPatients = useMemo(() => {
        ...
        const filtered = patientNames.filter(n => n.toLowerCase().includes(selectedPatientName.toLowerCase()))
        return filtered
    }, [patientNames, selectedPatientName])

    const fPractices = useMemo(() => {
        if (!selectedPractice) return practices
        const filtered = practices.filter(n => n.toLowerCase().includes(selectedPractice.toLowerCase()))
        return filtered
    }, [practices, selectedPractice])

    const fObrasSociales = useMemo(() => {
        if (!selectedObraSocial) return obrasSociales
        const filtered = obrasSociales.filter(n => n.toLowerCase().includes(selectedObraSocial.toLowerCase()))
        return filtered
    }, [obrasSociales, selectedObraSocial])

    const fInstituciones = useMemo(() => {
        if (!selectedInstitucion) return instituciones
        const filtered = instituciones.filter(n => n.toLowerCase().includes(selectedInstitucion.toLowerCase()))
        return filtered
                ...
                <div className="flex flex-col w-full relative">
                    ...
                    <div className="relative">
                        <DatePicker
                            ...
                            className="p-2 bg-white border border-gray-600 rounded-lg text-sm text-black w-full h-10 focus:ring-2 focus:ring-cyan-500 outline-none pr-8"
