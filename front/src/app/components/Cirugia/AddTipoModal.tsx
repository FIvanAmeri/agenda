...
interface Props {
  onClose: () => void;
  onAdded: (tipo: { id: number; nombre: string }) => void;
}

const AddTipoModal: React.FC<Props> = ({ onClose, onAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onAdded(data.tipo);
      alert("Tipo de cirugía agregado");
      close();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-md shadow-lg w-full max-w-md">
        <h3 className="text-lg font-semibold mb-4">Agregar Tipo de Cirugía</h3>
