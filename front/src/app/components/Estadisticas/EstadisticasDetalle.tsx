...
import { FaTimes, FaUser, FaCalendarAlt, FaMoneyBillWave, FaExclamationCircle } from "react-icons/fa";
...
const EstadisticasDetalle: React.FC = () => {
    ...
    const dataGraficoBarras = useMemo((): BarDataPoint[] => {
        ...
        if (!stats?.resumenPagos?.mensuales) {
            return nombresMeses.map(mes => ({ mes, monto: 0, pacientes: [] }));
        }
        return stats.resumenPagos.mensuales.map((dato, index) => ({
    ...
    const tooltipFormatter = (value: number | string | (number | string)[] | undefined) => {
        const numValue = typeof value === "number" ? value : 0;
        return [formatCurrency(numValue), "Monto"] as [string, string];
    };
        ...
        <div className="p-6 bg-gray-900 min-h-screen text-gray-100 overflow-y-auto relative">
            ...
            <div className="mb-8">
                <div className="bg-green-900/20 p-5 rounded-xl border border-green-700/50 inline-block min-w-[280px] shadow-lg">
                    <div className="flex items-center text-green-400 mb-1 gap-2">
                        ...
                        <span className="text-xs font-bold uppercase tracking-wider">Total Cobrado Real</span>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                <div className="bg-cyan-950 p-6 rounded-xl border border-gray-700 shadow-lg">
                    <h3 className="text-xl font-semibold mb-4 text-cyan-100 text-center">Honorarios por Mes de Cobro</h3>
                    <div className="h-80">
                                ...
                                <Bar 
                                    ...
                                    onClick={(data) => {
                                        if (data && data.payload) {
                                            ...
                                            openModal(`Cobros de ${payload.mes} ${selectedYear}`, payload.pacientes);
                                        }
                                    }}
                                />
                            </BarChart>
            ...
            <div className="space-y-8">
                <div className="bg-cyan-950 p-6 rounded-xl border border-gray-700 shadow-lg">
                    <h3 className="text-xl font-semibold mb-6 text-green-400 flex items-center gap-2">
                        <FaMoneyBillWave /> Prácticas Cobradas
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {Object.entries(stats.metricasPracticas).map(([clave, info]) => (
                            <div 
                                key={clave} 
                                onClick={() => openModal(clave, info.pacientes)}
                                className="bg-cyan-900/40 p-4 rounded-xl border border-cyan-800 flex items-center justify-between cursor-pointer hover:bg-cyan-800 transition-all hover:scale-[1.02]"
                            >
                ...
                <div className="bg-gray-900/50 p-6 rounded-xl border border-red-900/30 shadow-lg">
                    <h3 className="text-xl font-semibold mb-6 text-red-400 flex items-center gap-2">
                        <FaExclamationCircle /> Estudios No Pagados (Pendientes)
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {Object.entries(stats.metricasNoPagados).map(([clave, info]) => (
                            <div 
                                key={clave} 
                                onClick={() => openModal(`PENDIENTE: ${clave}`, info.pacientes)}
                                className="bg-red-950/20 p-4 rounded-xl border border-red-900/30 flex items-center justify-between cursor-pointer hover:bg-red-900/30 transition-all hover:scale-[1.02]"
                            >
                                <div className="flex flex-col min-w-0 flex-1">
            ...
            <div className="mt-8 bg-cyan-950 p-5 rounded-xl border border-gray-700 h-fit">
                <h3 className="text-lg font-semibold mb-3 text-cyan-100 border-b border-gray-700 pb-2 text-center">Obras Sociales (Estudios Cobrados)</h3>
