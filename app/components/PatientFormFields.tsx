...
export const PatientFormFields: React.FC<PatientFormFieldsProps> = ({
    ...
    const uniqueObrasSociales = Array.from(new Set(obrasSociales));
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3 overflow-visible">
            ...
            <div className="relative">
                ...
                {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute left-0 right-0 top-full mt-1 bg-[#0d2a35] border-2 border-cyan-500 rounded-md shadow-2xl z-50 max-h-60 overflow-y-auto">
                        {suggestions.map((p) => (
                            <div
                                key={p.id}
            ...
            <div>
                ...
                >
                    ...
                    {generarHoras().map((horaOption, index) => (
                        <option key={`${horaOption}-${index}`} value={horaOption}>{horaOption}</option>
                    ))}
                </select>
            </div>
            ...
            <div>
                ...
                >
                    ...
                    {uniqueObrasSociales.map((option, index) => (
            ...
            <div>
                ...
                >
                    ...
                    {INSTITUCION_OPTIONS.map((option, index) => (
                        <option key={`${option}-${index}`} value={option}>{option}</option>
                    ))}
                </select>
            </div>
            ...
            <div className="md:col-span-2">
                ...
                >
                    ...
                    {PRACTICAS_OPTIONS.map((option, index) => (
                        <option key={`${option}-${index}`} value={option}>{option}</option>
                    ))}
                </select>
            </div>
